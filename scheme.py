...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        if self.parent and symbol not in self.bindings:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while len(formals) != 0 and len(vals) != 0:
            Frame.define(new_frame, formals.first, vals.first)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        corresponding_expression = bindings.first.rest.first
        names = Pair(symbol, names)
        values = Pair(scheme_eval(corresponding_expression, env), values)
